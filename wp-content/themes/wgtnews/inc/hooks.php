...
if ( ! function_exists( 'understrap_add_site_info' ) ) {
	...
	function understrap_add_site_info() {
		...
		$site_info = sprintf(
			'© 2021 JNews -<a href="%1$s">%2$s</a> %3$s',
			esc_url( __( 'https://wordpress.org/', 'understrap' ) ),
			sprintf(
				/* translators: WordPress */
				esc_html__( 'Premium WordPress news %s', 'understrap' ),
				'WordPress'
			),
			sprintf( // WPCS: XSS ok.
				/* translators: 1: Theme name, 2: Theme author */
				esc_html__( '& %2$s.', 'understrap' ),
				$the_theme->get( 'Name' ),
				'<a href="' . esc_url( __( '#', 'understrap' ) ) . '">magazine theme by Jegtheme.</a>'
			),
			sprintf( // WPCS: XSS ok.
